...
function getDataForStatusInfoView(statusInfo) {
  ...
  let title = "";
  if (
    statusInfo.bittorrent &&
    statusInfo.bittorrent.info &&
    statusInfo.bittorrent.info.name
  ) {
  ...
  const speed = `⇣${utility.getAdjustedFormatBytes(
    statusInfo.downloadSpeed
  )}/s ⇡${utility.getAdjustedFormatBytes(statusInfo.uploadSpeed)}/s`;
  const size = `${utility.getAdjustedFormatBytes(statusInfo.totalLength)}(${
    statusInfo.numPieces
  } @ ${utility.getAdjustedFormatBytes(statusInfo.pieceLength)})`;
...
function getDataForFilesView(filesInfo) {
  return filesInfo.map(n => {
    return {
      ...
      size: {
        text: `${utility.getAdjustedFormatBytes(
          n.completedLength
...
function getDataForPeersView(peersInfo) {
  return peersInfo.map(n => {
    let client;
    try {
      const peerId = $text.URLDecode(
        utility.convertInvalidChrOfPeerId(n.peerId)
      );
