...
function setGlobalOptionToPrefs(result) {
  ...
  $prefs.set(
    ...
    parseInt(result["max-overall-download-limit"]) || 0
  );
  $prefs.set(
    "max-overall-upload-limit",
    parseInt(result["max-overall-upload-limit"]) || 0
  );
  $prefs.set(
    "max-concurrent-downloads",
    parseInt(result["max-concurrent-downloads"]) || 5
  );
...
async function changeGlobalOptionForServer() {
  const result = {
    dir: $prefs.get("dir"),
    "max-overall-download-limit": $prefs
      .get("max-overall-download-limit")
      .toString(),
    "max-overall-upload-limit": $prefs
      .get("max-overall-upload-limit")
      .toString(),
    "max-concurrent-downloads": $prefs
...
module.exports = {
  ...
  multicallRPC,
  changePrefs,
  getGlobalOptionFromServer,
  setGlobalOptionToPrefs,
  changeGlobalOptionForServer,
  getStatus,
  getVersion,
  convertInvalidChrOfPeerId
};
